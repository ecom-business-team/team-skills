...
A project has three end states. **Complete** — it shipped; the normal path, §1–§6. **Killed** — the owner decided it will not ship; §K. **Paused** — not an end state: work stopped waiting on a person or a decision; §P, and the project stays in flight. An **initiative** closes by §I once every milestone is shipped or killed.
...
2. **Reached:** milestone row → `reached {date}` (pointer to the evidence), in the initiative `state.md` (once the initiative has closed, the archived one, §I); remove the item from the tray (after a close, its line from **Outcome checks pending**); append an **Outcome** section to the archived `project_log.md` — what the memo predicted, what happened, the evidence — and turn the provisional lessons into final ones (what the Ship Review predicted against what real use showed). Promote anything general to `_practices/` or a standard. Close the task.
...
## I. Closing an initiative
An initiative closes when every milestone on its roadmap is **shipped or killed**. It does not wait for the outcome checks: each pending check is already a task that says where it records, and the archive keeps that place. Only on the owner's word.

1. **Empty the tray.** Every item in the handed-forward tray and every open point in `north_star.md` §10 leaves one of three ways: **done** (the step is taken now), **decided** (the owner rules now, and the ruling is written where it belongs), or **homed** (a task in the task manager's project carrying its why, pointers and next action, or a line in the standard, skill, practice or system `CONTEXT.md` that owns it). Homing is the default; nothing is decided just to empty the tray. A held switch is published or becomes a task. A verified fact moves to the `CONTEXT.md` of the system it describes.
2. **Sweep the task manager's project.** Close every task already done or superseded, each with a one-line reason. The project stays as the backlog of what the initiative built; its description points at the system's `CONTEXT.md`, not the planning folder.
3. **Repoint the pending checks.** Settle the archive name first, by step 5's rule. Every open outcome-check task names the archived paths: its milestone row in `_admin/_archive/<initiative>/state.md`, and any input it reads from the planning folder. After the close, a check sets its row there and removes its line from **Outcome checks pending**, then re-renders the page (the one kind of edit an archived initiative accepts); the rest of "Running the check" is unchanged.
4. **Write the close record.** Rewrite `state.md` as the final snapshot: `**Closed:** {date}` replaces the `**Updated:**` line; In flight → none; the milestone table as it stands (shipped rows keep their check dates), its relative links rewritten for the folder's new place one level deeper; the tray replaced by **Where everything went** (one line per item: its home and task id); and **Outcome checks pending** (milestone · due · task id). Render its companion as in §4.
5. **Archive.** Move the folder to `_admin/_archive/<initiative>/` (if that name is taken, `<initiative>-initiative/`). Move, never copy; verify with `ls`.
6. **Repoint the entries.** Find every live pointer by searching the workspace and the task manager for the folder's path (the workspace's routing file, the parent `_admin/CONTEXT.md`, task descriptions, the task-manager project's description and memory, for example), and point each at the archived folder or at the system's `CONTEXT.md`. The folder's own `CONTEXT.md` says it closed and names the close record. The archived projects' own pointers are history and stay as written.
7. **Handoff card** (template §4.10): **Ribbon** every milestone row with its status; **Done** = the initiative closed, proved by `ls` and the close record; **Needs {owner}** = the pending checks, dated; **Next** = the first pending check's date; **Written** = the close record.
