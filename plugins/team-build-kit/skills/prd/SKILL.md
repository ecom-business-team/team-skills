...
5. **A rule and the texts that apply it are checked against each other.** When the proposed changes write a rule (a template, a status vocabulary, a shared procedure) and also the texts that apply it (each skill's values, each printer's lines), record one validation-log row per applying text that quotes the rule and confirms the text obeys it. Why: a PRD once wrote a card rule and ten skills' card values in the same pass; two of the values contradicted the rule, and nothing compared them until a cold reader did at the end of the build (milestone 10, 2026-09-22).
6. **A cold-read proof is planned before it runs.** When the build is proved by a cold read (a procedure always; knowledge when the memo asks that a fresh reader can build from it), name in the PRD one brief per kind of consumer the document serves, with a reason for any kind left out, and the stopping rule (default: two consecutive rounds at ≤1 new item per brief, each answered). Record one validation-log row that lists the briefs and the rule. The rule lives in `documentation_standard.md` §4, "When a cold read is the proof". Why: milestone 1.2's proof tuned the document to its only brief (2026-09-23).
